...
class IMAPManager:
    ...
    def move_email(self, client: IMAPClient, msg_id: int, target_folder: str) -> bool:
        ...
        """
        try:
            logger.debug(f"Moving email {msg_id} to {target_folder}")
            ...
            if is_unread:
                logger.debug(f"Preserving unread status for {msg_id} in {target_folder}")
                ...
                if messages:
                    ...
                    logger.debug(f"Preserved unread status for {len(messages)} emails")
                else:
                    logger.warning(f"Could not find email {msg_id} in {target_folder} to preserve unread status")
            
            return True
        except Exception as e:
            logger.error(f"Error moving email {msg_id} to {target_folder}: {e}", exc_info=True)
            return False 